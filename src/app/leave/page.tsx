...
export default function LeavePage() {
    const { requests, policies, balances: storeBalances, addRequest, updateStatus, getEmployeeBalances, addPolicy, updatePolicy, deletePolicy } = useLeaveStore();
    ...
    const filteredRequests = requests.filter((r) => {
        ...
        if (isEmployee && myEmpId && r.employeeId !== myEmpId) return false;
        return true;
    });

    const handleSubmit = () => {
        if (canApprove && !formEmpId) {
        ...
        const empId = formEmpId || myEmpId || "EMP001";
    ...
    const handleApprove = (id: string, employeeId: string, startDate: string, endDate: string) => {
        ...
        // Sync leave→attendance: mark days as on_leave in attendance store
                            ...
                            <CardContent className="p-4">
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-lg font-bold">{isEmployee ? b.remaining : b.used}</p>
                                    </div>
                                </div>
                                ...
                                <p className="text-[10px] text-muted-foreground mt-1">
                                    {isEmployee ? `${b.used} used / ${b.alloc} total` : `${b.used} days used org-wide`}
                                </p>
                            </CardContent>
                        </Card>
                    );
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {filteredRequests.length === 0 ? (
                                <TableRow><TableCell colSpan={canApprove ? 8 : 7} className="text-center text-sm text-muted-foreground py-8">No leave requests</TableCell></TableRow>
