...
// Persist org settings in localStorage
...
export default function SettingsPage() {
    ...
    const handleCreateUser = () => {
        ...
        const result = createAccount(
            { name: newUserName, email: newUserEmail, role: newUserRole, password: newUserPw, mustChangePassword: newUserMustChange },
            currentUser.email
        );
        if (!result.ok) { toast.error(result.error); return; }
        toast.success(`Account created for ${newUserName}.`);
        setNewUserName(""); setNewUserEmail(""); setNewUserPw(""); setNewUserRole("employee");
    ...
    const handleOpenAdd = () => {
        // Reset to defaults
        setNewName("");
        setNewStandardHours("8");
        setNewGraceMinutes("10");
        setNewRoundingPolicy("nearest_15");
        setNewOTRequired(true);
        setNewNightDiffStart("22:00");
        setNewNightDiffEnd("06:00");
        setNewHolidayMultiplier("2.0");
        setAddRuleOpen(true);
    };

    const handleCreateNew = () => {
        if (!newName || !newStandardHours || !newGraceMinutes) {
        ...
        toast.success(`Rule set "${newName}" created successfully`);
    ...
    const handleOpenEdit = (rule: AttendanceRuleSet) => {
        ...
        setEditStandardHours(String(rule.standardHoursPerDay));
        setEditGraceMinutes(String(rule.graceMinutes));
        setEditRoundingPolicy(rule.roundingPolicy);
        setEditOTRequired(rule.overtimeRequiresApproval);
        setEditNightDiffStart(rule.nightDiffStart || "22:00");
        setEditNightDiffEnd(rule.nightDiffEnd || "06:00");
        setEditHolidayMultiplier(String(rule.holidayMultiplier));
        setEditRuleOpen(true);
    };

    const handleSaveEdit = () => {
        if (!editingRule) return;
        if (!editName || !editStandardHours || !editGraceMinutes) {
            toast.error("Please fill all required fields");
            return;
        }

        updateRuleSet(editingRule.id, {
            name: editName,
            standardHoursPerDay: Number(editStandardHours),
            graceMinutes: Number(editGraceMinutes),
            roundingPolicy: editRoundingPolicy,
            overtimeRequiresApproval: editOTRequired,
            nightDiffStart: editNightDiffStart,
            nightDiffEnd: editNightDiffEnd,
            holidayMultiplier: Number(editHolidayMultiplier),
        });
        ...
        <div className="space-y-6 max-w-3xl">
                            ...
                            <CardContent className="p-4">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-sm font-semibold group-hover:text-blue-600 transition-colors">Org Structure</p>
                                            <p className="text-xs text-muted-foreground">Departments &amp; positions</p>
                                        </div>
                                    </div>
                            ...
                            <CardContent className="p-4">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-sm font-semibold group-hover:text-purple-600 transition-colors">Shifts &amp; Time</p>
                                            <p className="text-xs text-muted-foreground">Shift templates &amp; assignments</p>
                                        </div>
                                    </div>
                ...
                <Card className="border border-border/50">
                    <CardHeader className="pb-3">
                        <div className="flex items-center gap-2">
                            ...
                            <CardTitle className="text-base font-semibold">Pay Schedule</CardTitle>
                        </div>
                    ...
                    <CardContent className="space-y-5">
                        ...
                        <div>
                            ...
                            <Select
                                value={paySchedule.defaultFrequency}
                                onValueChange={(v) => { updatePaySchedule({ defaultFrequency: v as PayFrequency }); toast.success(`Default frequency set to ${v.replace("_", "-")}`); }}
                            >
                                <SelectTrigger className="mt-1.5"><SelectValue /></SelectTrigger>
                                <SelectContent>
                                    <SelectItem value="monthly">Monthly (1× per month)</SelectItem>
                                    <SelectItem value="semi_monthly">Semi-Monthly (2× per month)</SelectItem>
                            ...
                            <div className="p-3 rounded-lg border border-border/50 space-y-3">
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    <div>
                                        ...
                                        <p className="text-[10px] text-muted-foreground mt-0.5">e.g. 15 → 1st–15th</p>
                                    </div>
                                    <div>
                                        <label className="text-xs text-muted-foreground">1st Pay Day</label>
                                        ...
                                        <p className="text-[10px] text-muted-foreground mt-0.5">Pay release day</p>
                                    </div>
                                    <div>
                                        <label className="text-xs text-muted-foreground">2nd Pay Day</label>
                                        ...
                                        <p className="text-[10px] text-muted-foreground mt-0.5">Of next month</p>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <Select value={paySchedule.deductGovFrom} onValueChange={(v) => updatePaySchedule({ deductGovFrom: v as "first" | "second" | "both" })}>
                                        ...
                                        <SelectContent>
                                            ...
                                            <SelectItem value="both">Split Across Both</SelectItem>
                                        </SelectContent>
                                    </Select>
                        ...
                        {paySchedule.defaultFrequency === "monthly" && (
                            <div className="p-3 rounded-lg border border-border/50 space-y-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-muted-foreground mt-0.5">Day of month salary is released</p>
                                </div>
                            </div>
                        )}
                        ...
                        {paySchedule.defaultFrequency === "bi_weekly" && (
                            <div className="p-3 rounded-lg border border-border/50 space-y-3">
                                ...
                                <div>
                                    <label className="text-xs text-muted-foreground">Reference Start Date</label>
                                    ...
                                    <p className="text-[10px] text-muted-foreground mt-0.5">First pay period starts on this date, then every 2 weeks</p>
                                </div>
                            </div>
                        )}
                ...
                <CardContent>
                    <div className="space-y-3">
                        {[
                            ...
                            { role: "Supervisor", desc: "Timesheet approval, team oversight", color: "bg-cyan-500/15 text-cyan-700 dark:text-cyan-400" },
                            { role: "Payroll Admin", desc: "Payroll processing, deduction management", color: "bg-violet-500/15 text-violet-700 dark:text-violet-400" },
                            { role: "Auditor", desc: "Read-only audit trail access", color: "bg-orange-500/15 text-orange-700 dark:text-orange-400" },
                ...
                <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <CardTitle className="text-base font-semibold">Timesheet Rule Sets</CardTitle>
                        </div>
                        {isAdmin && (
                            <Button
                                variant="outline"
                        ...
                        {ruleSets.map((rs) => (
                            <div key={rs.id} className="p-3 rounded-lg border border-border/50 space-y-2">
                                ...
                                <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                                    <span>Standard Hours: {rs.standardHoursPerDay}h/day</span>
                                    <span>Grace Period: {rs.graceMinutes}min</span>
                                    <span>Rounding: {rs.roundingPolicy.replace("_", " ")}</span>
                                    <span>OT Approval: {rs.overtimeRequiresApproval ? "Required" : "None"}</span>
                                    <span>Night Diff: {rs.nightDiffStart || "N/A"} — {rs.nightDiffEnd || "N/A"}</span>
                                    <span>Holiday Mult: {rs.holidayMultiplier}x</span>
                                </div>
                            </div>
                        ))}
                        {ruleSets.length === 0 && (
                            <p className="text-sm text-muted-foreground text-center py-4">No rule sets configured. Visit Timesheets to add one.</p>
                        )}
                    </div>
                </CardContent>
            </Card>
            ...
            <Dialog open={editRuleOpen} onOpenChange={setEditRuleOpen}>
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>Edit Timesheet Rule Set — {editingRule?.id}</DialogTitle>
                    </DialogHeader>
                    <div className="space-y-4 pt-2">
                        <div>
                            ...
                            <Input
                                ...
                                placeholder="e.g., Standard PH Rule Set"
                                className="mt-1"
                            />
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <div>
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setEditGraceMinutes(e.target.value)}
                                    className="mt-1"
                                />
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <Select value={editRoundingPolicy} onValueChange={(v) => setEditRoundingPolicy(v as any)}>
                            ...
                            <div>
                                ...
                                <Input
                                    ...
                                    max="5"
                                    step="0.1"
                                    value={editHolidayMultiplier}
                                    onChange={(e) => setEditHolidayMultiplier(e.target.value)}
                                    className="mt-1"
                                />
                            </div>
                        </div>
                        <div className="flex items-center justify-between p-3 rounded-lg border border-border/50">
                            <div>
                                <p className="text-sm font-medium">Overtime Requires Approval</p>
                                <p className="text-xs text-muted-foreground">OT hours must be pre-approved before counting</p>
                            </div>
                            <Switch
                                checked={editOTRequired}
                                onCheckedChange={setEditOTRequired}
                            />
                        </div>
                        <div>
                            <label className="text-sm font-medium">Night Differential Hours</label>
                            <p className="text-xs text-muted-foreground mb-2">Hours between these times get night diff premium</p>
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="text-xs text-muted-foreground">Start Time</label>
                                    <Input
                                        type="time"
                                        value={editNightDiffStart}
                                        onChange={(e) => setEditNightDiffStart(e.target.value)}
                                        className="mt-1"
                                    />
                                </div>
                                <div>
                                    ...
                                    <Input
                                        type="time"
                                        value={editNightDiffEnd}
                                        onChange={(e) => setEditNightDiffEnd(e.target.value)}
                                        className="mt-1"
                                    />
                                </div>
                            </div>
            ...
            <Dialog open={addRuleOpen} onOpenChange={setAddRuleOpen}>
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>Create New Timesheet Rule Set</DialogTitle>
                    </DialogHeader>
                    <div className="space-y-4 pt-2">
                        <div>
                            <label className="text-sm font-medium">Rule Set Name *</label>
                            <Input
                                ...
                                placeholder="e.g., Night Shift Rule Set, Flexible Hours"
                                className="mt-1"
                            />
                            <p className="text-xs text-muted-foreground mt-1">Give it a descriptive name for the shift type</p>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <label className="text-sm font-medium">Standard Hours/Day *</label>
                                <Input
                                    ...
                                    max="24"
                                    step="0.5"
                                    value={newStandardHours}
                                    onChange={(e) => setNewStandardHours(e.target.value)}
                                    className="mt-1"
                                />
                                <p className="text-xs text-muted-foreground mt-1">Regular work hours per day</p>
                            </div>
                            <div>
                                <label className="text-sm font-medium">Grace Period (minutes) *</label>
                                <Input
                                    ...
                                    min="0"
                                    max="60"
                                    value={newGraceMinutes}
                                    onChange={(e) => setNewGraceMinutes(e.target.value)}
                                    className="mt-1"
                                />
                                <p className="text-xs text-muted-foreground mt-1">Late allowance before marking late</p>
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                <label className="text-sm font-medium">Rounding Policy</label>
                                <Select value={newRoundingPolicy} onValueChange={(v) => setNewRoundingPolicy(v as any)}>
                                    <SelectTrigger className="mt-1">
                                        <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                                ...
                                <p className="text-xs text-muted-foreground mt-1">How to round worked hours</p>
                            </div>
                            <div>
                                <label className="text-sm font-medium">Holiday Multiplier</label>
                                <Input
                                    ...
                                    className="mt-1"
                                />
                                <p className="text-xs text-muted-foreground mt-1">Pay multiplier for holidays (e.g., 2.0x)</p>
                            </div>
                        </div>
                        <div className="flex items-center justify-between p-3 rounded-lg border border-border/50">
                            <div>
                            ...
                            <Switch
                                checked={newOTRequired}
                                onCheckedChange={setNewOTRequired}
                            />
                        </div>
                        <div>
                            <label className="text-sm font-medium">Night Differential Hours (Optional)</label>
                            <p className="text-xs text-muted-foreground mb-2">Hours between these times get night diff premium (e.g., 10% extra)</p>
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="text-xs text-muted-foreground">Start Time</label>
                                    <Input
                                        type="time"
                                        value={newNightDiffStart}
                                        onChange={(e) => setNewNightDiffStart(e.target.value)}
                                        className="mt-1"
                                    />
                                </div>
                                <div>
                                    ...
                                    <Input
                                        type="time"
                                        value={newNightDiffEnd}
                                        onChange={(e) => setNewNightDiffEnd(e.target.value)}
                                        className="mt-1"
                                    />
                                </div>
                            </div>
                        ...
                        <div className="p-3 rounded-lg bg-blue-500/5 border border-blue-500/20">
                            ...
                            <div className="grid grid-cols-2 gap-2 mt-2">
                                <Button
                                    ...
                                    onClick={() => {
                                        setNewName("Night Shift Rule Set");
                                        setNewStandardHours("8");
                                        setNewGraceMinutes("15");
                                        setNewNightDiffStart("22:00");
                                        setNewNightDiffEnd("06:00");
                                    }}
                                >
                                ...
                                <Button
                                    ...
                                    className="text-xs h-8"
                                    onClick={() => {
                                        setNewName("Flexible Hours Rule Set");
                                        setNewStandardHours("6");
                                        setNewGraceMinutes("30");
                                        setNewRoundingPolicy("none");
                                    }}
                                >
                                    Flexible (6h, 30min grace)
                                </Button>
                                <Button
                                    variant="outline"
                                    size="sm"
                                    className="text-xs h-8"
                                    onClick={() => {
                                        setNewName("12-Hour Shift Rule Set");
                                        setNewStandardHours("12");
                                        setNewGraceMinutes("10");
                                    }}
                                >
                                    12-Hour Shift
                                ...
                                <Button
                                    ...
                                    className="text-xs h-8"
                                    onClick={() => {
                                        setNewName("Part-Time Rule Set");
                                        setNewStandardHours("4");
                                        setNewGraceMinutes("5");
                                        setNewOTRequired(false);
                                    }}
                                >
                                    Part-Time (4h)
                                </Button>
            ...
            <Card className="border border-border/50">
                ...
                <CardContent className="space-y-4">
                    {[
                        { key: "emailAbsenceAlerts" as const, label: "Absence alerts", desc: "Email when an employee is absent" },
                        { key: "emailLeaveUpdates" as const, label: "Leave updates", desc: "Email when leave is approved/rejected" },
                        { key: "emailPayrollAlerts" as const, label: "Payroll alerts", desc: "Email when payslips are issued" },
                    ].map((n) => (
                        <div key={n.key} className="flex items-center justify-between">
                            ...
                            <Switch
                                ...
                                onCheckedChange={(checked) => { update({ [n.key]: checked }); toast.success(`${n.label} ${checked ? "enabled" : "disabled"}`); }}
                            />
                        </div>
                    ))}
                </CardContent>
                ...
                <Card className="border border-border/50">
                    <CardHeader className="pb-3">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                <Shield className="h-5 w-5 text-muted-foreground" />
                                <CardTitle className="text-base font-semibold">User Accounts</CardTitle>
                            </div>
                            <Button size="sm" onClick={() => setAddUserOpen(true)}>
                                <UserPlus className="w-4 h-4 mr-1.5" /> Add Account
                            </Button>
                        </div>
                    </CardHeader>
                            ...
                            <div key={acc.id} className="flex items-center justify-between p-2.5 rounded-lg border border-border/50 hover:bg-muted/30 transition-colors">
                                ...
                                <div className="flex items-center gap-2 shrink-0 ml-2">
                                    ...
                                    {acc.mustChangePassword && (
                                        <Badge variant="outline" className="text-[10px] text-amber-600 border-amber-300">pw reset</Badge>
                                    )}
                                    {!acc.profileComplete && (
                                        <Badge variant="outline" className="text-[10px] text-blue-600 border-blue-300">onboarding</Badge>
                                    )}
                                    <Button
                                        size="icon"
                                        variant="ghost"
                                    ...
                                    {acc.id !== currentUser.id && (
                                        <Button
                                            ...
                                            onClick={() => { deleteAccount(acc.id); toast.success(`Account for ${acc.name} deleted.`); }}
                                        >
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">
                                    Wipes every store (employees, attendance, payroll, leave, loans, projects,
                                    timesheets, events, notifications &amp; accounts) and restores seed data.
                            ...
                            >
                                <RotateCcw className="w-3.5 h-3.5 mr-1.5" /> Reset All Data
                            </Button>
                        </div>
                    </CardContent>
                </Card>
                ...
                <DialogContent className="sm:max-w-sm">
                    ...
                    <div className="space-y-3 pt-1">
                        <div className="space-y-1.5">
                            ...
                            <Input value={newUserName} onChange={(e) => setNewUserName(e.target.value)} placeholder="e.g. Maria Santos" />
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-sm font-medium">Email *</label>
                            <Input type="email" value={newUserEmail} onChange={(e) => setNewUserEmail(e.target.value)} placeholder="maria@nexhrms.com" />
                        </div>
                        <div className="space-y-1.5">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <Input type="password" value={newUserPw} onChange={(e) => setNewUserPw(e.target.value)} placeholder="Min. 6 characters" />
                        </div>
                        <div className="flex items-center justify-between pt-1">
                            <div>
                                <p className="text-sm font-medium">Require password change</p>
                                <p className="text-xs text-muted-foreground">User must create a new password on first login</p>
                            </div>
                            <Switch checked={newUserMustChange} onCheckedChange={setNewUserMustChange} />
                        </div>
                ...
                <DialogContent className="sm:max-w-sm">
                    ...
                    <div className="space-y-3 pt-1">
                        <p className="text-sm text-muted-foreground">
                            Set a new temporary password for <strong>{accounts.find((a) => a.id === resetPwUserId)?.name}</strong>.
            ...
            <AlertDialogContent>
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        This will permanently wipe all data across every module (employees, attendance,
                        payroll, leave, loans, projects, timesheets, events, notifications, and accounts)
                        and restore the original seed / demo state. You will be logged out immediately.
                        <br /><br />
                ...
                <AlertDialogFooter>
                    ...
                    >
                        <RotateCcw className="w-3.5 h-3.5 mr-1.5" /> Yes, Reset Everything
                    </AlertDialogAction>
                </AlertDialogFooter>
            </AlertDialogContent>
        </AlertDialog>
