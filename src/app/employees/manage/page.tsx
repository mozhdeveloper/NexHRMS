...
export default function EmployeesManagePage() {
    ...
    // Salary governance state (Directory tab)
    ...
    const handleAddEmployee = () => {
        ...
        if (!newName || !newEmail || !newRole || !newDept) {
        ...
        if (newPassword) {
            const result = createAccount(
                { name: newName, email: newEmail, role: newSystemRole, password: newPassword, mustChangePassword: newMustChange, profileComplete: true },
                currentUser.email
            );
            if (!result.ok) {
                toast.warning(`Employee added but account creation failed: ${result.error}`);
            } else {
        ...
        if (newProjectId && newProjectId !== "none") {
            assignToProject(newProjectId, id);
        }
    ...
    const handleOpenEdit = (emp: Employee) => {
        ...
        setEditWorkType(emp.workType);
        setEditSalary(String(emp.salary));
        setEditPhone(emp.phone || "");
        setEditLocation(emp.location);
        setEditProductivity(String(emp.productivity));
        setEditPayFreq(emp.payFrequency || "company");
    ...
    const handleSaveEdit = () => {
        ...
        updateEmployee(editingEmp.id, {
            ...
            productivity: Number(editProductivity) || 80,
        ...
        if (currentProject && currentProject.id !== editProjectId) {
            ...
            removeFromProject(currentProject.id, editingEmp.id);
        }
        ...
        } else if (editProjectId === "none" && currentProject) {
            // Remove from project
            removeFromProject(currentProject.id, editingEmp.id);
        }
                ...
                <Dialog open={addOpen} onOpenChange={setAddOpen}>
                    ...
                    <DialogContent className="max-w-2xl p-0 gap-0 overflow-hidden">
                        ...
                        <div className="overflow-y-auto max-h-[calc(85vh-160px)] px-6 py-4 space-y-4">
                                ...
                                <div className="p-4 space-y-3">
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <Input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Juan dela Cruz" className="mt-1 h-8 text-sm" />
                                        </div>
                                        <div>
                                            <label className="text-xs font-medium text-muted-foreground">Email Address <span className="text-destructive">*</span></label>
                                            <Input type="email" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} placeholder="juan@company.com" className="mt-1 h-8 text-sm" />
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            <label className="text-xs font-medium text-muted-foreground">Phone</label>
                                            <Input value={newPhone} onChange={(e) => setNewPhone(e.target.value)} placeholder="+63 912 345 6789" className="mt-1 h-8 text-sm" />
                                        </div>
                                        <div>
                                            <label className="text-xs font-medium text-muted-foreground">Office Location</label>
                                            <Select value={newLocation} onValueChange={setNewLocation}>
                                                <SelectTrigger className="mt-1 h-8 text-sm"><SelectValue placeholder="Select office" /></SelectTrigger>
                                                <SelectContent>
                            ...
                            <div className="rounded-lg border bg-card">
                                ...
                                <div className="p-4 space-y-3">
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            <label className="text-xs font-medium text-muted-foreground">Job Title <span className="text-destructive">*</span></label>
                                        ...
                                        <div>
                                            ...
                                            <Select value={newWorkType} onValueChange={(v) => setNewWorkType(v as WorkType)}>
                                                ...
                                                <SelectContent>
                                                    ...
                                                    <SelectItem value="ONSITE">Full Onsite</SelectItem>
                                                </SelectContent>
                                            </Select>
                                        </div>
                                        <div>
                                            <label className="text-xs font-medium text-muted-foreground">Monthly Salary (₱)</label>
                                            <Input type="number" value={newSalary} onChange={(e) => setNewSalary(e.target.value)} placeholder="e.g. 25000" className="mt-1 h-8 text-sm" />
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            <label className="text-xs font-medium text-muted-foreground">Pay Frequency</label>
                                            <Select value={newPayFreq} onValueChange={setNewPayFreq}>
                                                <SelectTrigger className="mt-1 h-8 text-sm"><SelectValue /></SelectTrigger>
                                                <SelectContent>
                                    ...
                                    <div>
                                        <div className="flex items-center justify-between mb-2">
                                            <label className="text-xs font-medium text-muted-foreground">
                                                Work Days <span className="text-muted-foreground/60 font-normal">(optional)</span>
                                            </label>
                                            <div className="flex items-center gap-1">
                                                {[
                                                    { label: "Mon–Fri", days: ["Mon","Tue","Wed","Thu","Fri"] },
                                                    { label: "Mon–Sat", days: ["Mon","Tue","Wed","Thu","Fri","Sat"] },
                                                    { label: "All 7", days: ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"] },
                                                    { label: "Weekends", days: ["Sat","Sun"] },
                                                ].map(({ label, days }) => (
                                                    <button
                                                        key={label}
                                                        type="button"
                                                        onClick={() => setNewWorkDays(days)}
                                                        className="px-2 py-0.5 text-[10px] font-medium rounded border border-border text-muted-foreground hover:bg-muted hover:text-foreground transition-colors"
                                                    >
                                                        {label}
                                                    </button>
                                                ))}
                                                <button
                                                    type="button"
                                                    onClick={() => setNewWorkDays([])}
                                                    className="px-2 py-0.5 text-[10px] font-medium rounded border border-dashed border-muted-foreground/30 text-muted-foreground hover:text-destructive hover:border-destructive/50 transition-colors"
                                                >
                                                    Clear
                                                </button>
                                            </div>
                                        ...
                                        {newWorkDays.length > 0 && (
                                            <p className="text-[11px] text-muted-foreground mt-1.5">
                                                {newWorkDays.length} day{newWorkDays.length !== 1 ? "s" : ""} selected &mdash; {newWorkDays.join(", ")}
                                            </p>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="p-4 space-y-3">
                                    {newPassword ? (
                                        <div className="flex items-center gap-2 rounded-md bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 p-2.5 text-xs text-emerald-800 dark:text-emerald-300">
                                            ...
                                            <span>Account will be created — employee can log in immediately after being added.</span>
                                        </div>
                                    ) : (
                                        <div className="flex items-start gap-2 rounded-md bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 p-2.5 text-xs text-amber-800 dark:text-amber-300">
                                            ...
                                            <span>No password set — this employee <strong>won&apos;t be able to log in</strong>. Set a password or generate one below.</span>
                                        </div>
                                    )}
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            <label className="text-xs font-medium text-muted-foreground">System Role</label>
                                            <Select value={newSystemRole} onValueChange={(v) => setNewSystemRole(v as Role)}>
                                                <SelectTrigger className="mt-1 h-8 text-sm"><SelectValue /></SelectTrigger>
                                                <SelectContent>
                                        ...
                                        <div>
                                            ...
                                            <div className="flex gap-1.5 mt-1">
                                                ...
                                                <button
                                                    ...
                                                    className="shrink-0 rounded-md border h-8 px-2 text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
                                                >
                                    ...
                                    <div className={`flex items-center justify-between rounded-md border px-3 py-2 transition-colors ${newPassword ? "bg-background" : "bg-muted/30 opacity-50"}`}>
                                        <div>
                                            <p className="text-xs font-medium">Require password change on first login</p>
                                            <p className="text-[11px] text-muted-foreground">Prompts employee to set their own password</p>
                                        </div>
                            ...
                            <div className="rounded-lg border bg-card">
                                ...
                                <div className="p-4">
                                    ...
                                    {newProjectId && newProjectId !== "none" && (() => {
                                        ...
                                        return proj ? (
                                            <p className="text-[11px] text-muted-foreground mt-1.5">
                                                {proj.assignedEmployeeIds?.length ?? 0} member{(proj.assignedEmployeeIds?.length ?? 0) !== 1 ? "s" : ""} currently on this project
                                            </p>
                                        ) : null;
                                    })()}
                                </div>
                        ...
                        <div className="flex items-center justify-end gap-2 px-6 py-4 border-t bg-muted/20">
                            ...
                            <Button onClick={handleAddEmployee} className="gap-1.5 h-8 text-sm">
                    ...
                    <div className="space-y-4 pt-2">
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <Input value={editName} onChange={(e) => setEditName(e.target.value)} placeholder="John Doe" className="mt-1" />
                            </div>
                            <div>
                                <label className="text-sm font-medium">Email *</label>
                                <Input type="email" value={editEmail} onChange={(e) => setEditEmail(e.target.value)} placeholder="john@company.com" className="mt-1" />
                            </div>
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                            <div>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            ...
                            <div>
                                ...
                                <Input type="number" value={editSalary} onChange={(e) => setEditSalary(e.target.value)} placeholder="e.g. 85000" className="mt-1" />
                            </div>
                            <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <div>
                                ...
                                <Input value={editPhone} onChange={(e) => setEditPhone(e.target.value)} placeholder="+1-555-0123" className="mt-1" />
                            </div>
                            <div>
                                <label className="text-sm font-medium">Productivity (%)</label>
                                <Input type="number" min="0" max="100" value={editProductivity} onChange={(e) => setEditProductivity(e.target.value)} placeholder="80" className="mt-1" />
                            </div>
                        </div>
                        <div>
                            <label className="text-sm font-medium">Assigned Project</label>
                            <Select value={editProjectId || "none"} onValueChange={setEditProjectId}>
                                <SelectTrigger className="mt-1"><SelectValue placeholder="Select project" /></SelectTrigger>
                                <SelectContent>
                                    <SelectItem value="none">No Project</SelectItem>
                                    {projects.filter(p => p.status !== "completed").map((p) => (
                                        <SelectItem key={p.id} value={p.id}>
                                            {p.name} {p.assignedEmployeeIds.includes(editingEmp?.id || "") ? "✓" : ""}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                            <p className="text-xs text-muted-foreground mt-1">
                                Assigned project defines geofence for attendance check-in
                            </p>
                        </div>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    {visibleCols.salary && <TableHead className="cursor-pointer text-xs" onClick={() => handleSort("salary")}>Salary (Monthly)<SortIcon col="salary" /></TableHead>}
                            ...
                            <TableBody>
                                {paginated.map((emp) => {
                                    ...
                                    <TableRow key={emp.id} className="group">
                                        ...
                                        {visibleCols.status && (
                                            <TableCell>
                                                <Badge variant="secondary" className={emp.status === "active" ? "bg-emerald-500/15 text-emerald-700 dark:text-emerald-400" : emp.status === "resigned" ? "bg-orange-500/15 text-orange-700 dark:text-orange-400" : "bg-red-500/15 text-red-700 dark:text-red-400"} >
                                                    {emp.status}
                                                </Badge>
                                            </TableCell>
                                        )}
                                        ...
                                        {visibleCols.project && (
                                            <TableCell className="text-xs">
                                                {assignedProject ? (
                                                    <Badge variant="outline" className="text-[10px] bg-blue-500/10 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-800">
                                        ...
                                        <TableCell>
                                            <div className="flex items-center gap-1">
                                                ...
                                                {canManage && emp.status === "active" && (
                                                    <AlertDialog>
                                                        <AlertDialogTrigger asChild>
                                                            <Button variant="ghost" size="icon" className="h-7 w-7 text-orange-500 hover:text-orange-700 hover:bg-orange-500/10" title="Resign">
                                                        ...
                                                        <AlertDialogContent>
                                                            <AlertDialogHeader>
                                                                ...
                                                                <AlertDialogDescription>
                                                                    This will mark <strong>{emp.name}</strong> as resigned and compute their final pay including pro-rated salary, leave conversion, and loan offset.
                                                                </AlertDialogDescription>
                                                            </AlertDialogHeader>
                                                            <AlertDialogFooter>
                                                ...
                                                {canManage && emp.status === "inactive" && (
                                                    <AlertDialog>
                                                        <AlertDialogTrigger asChild>
                                                            <Button variant="ghost" size="icon" className="h-7 w-7 text-red-500 hover:text-red-700 hover:bg-red-500/10">
                                                        ...
                                                        <AlertDialogContent>
                                                            <AlertDialogHeader>
                                                                ...
                                                                <AlertDialogDescription>
                                                                    Are you sure you want to permanently remove <strong>{emp.name}</strong>? This action cannot be undone.
                                                                </AlertDialogDescription>
                                                            </AlertDialogHeader>
                                                            <AlertDialogFooter>
                                                                <AlertDialogCancel>Cancel</AlertDialogCancel>
                                                                <AlertDialogAction
                                                                    ...
                                                                    onClick={() => { removeEmployee(emp.id); toast.success(`${emp.name} removed`); }}
                                                                >
                                                                    Delete
                                                                </AlertDialogAction>
                                                            </AlertDialogFooter>
                                        ...
                                        <CardContent className="p-5">
                                            ...
                                            <div className="mt-4 space-y-2">
                                                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                                    <Mail className="h-3.5 w-3.5 shrink-0" /><span className="truncate">{emp.email}</span>
                                                </div>
                                                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                                    <MapPin className="h-3.5 w-3.5 shrink-0" /><span>{emp.location}</span>
                                                </div>
                                                {emp.phone && (
                                                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                                        <Phone className="h-3.5 w-3.5 shrink-0" /><span>{emp.phone}</span>
                                                    </div>
                                                )}
                                                {emp.birthday && (
                                                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    <div className="space-y-3 py-2">
                        ...
                        {isHR && (
                            <div>
                                ...
                                <Input value={salaryReason} onChange={(e) => setSalaryReason(e.target.value)} placeholder="e.g. Annual performance review" className="mt-1" />
                            </div>
                        )}
                    </div>
                    <DialogFooter>
